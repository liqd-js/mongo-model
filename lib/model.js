...
const { Filter, Projection, UnderscoredFirst } = require('./helpers');
...
module.exports = class Model extends EventEmitter
{
    ...
    async update( id, modification, options = {})
    {
        ...
        do
        {
            ...
            entry = await this.#collection.findOneAndUpdate({ ...filter, $or: [{ '__model.lock': { $lt: getLockTimeout()}}, { '__model.lock': options.key }]}, modification ).then( r => r.value );
        }
        while( !entry && await SLEEP( 100 ));
