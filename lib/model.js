...
{
    #collection; #name; #options; #get_aggregator;
    ...
    async create( id, data, options = {})
    {
        ...
        let now = Date.now(), key;
        let entry = UnderscoredFirst({ _id: id = ( id || randomID()), ...data, __model: { version: 1, created: now, updated: now, lock: options.lock ? ( key = generateLockKey( typeof options.lock === 'number' ? options.lock : undefined )) : 0 }});

        let insert = await this.#collection.insertOne( entry ).catch( err =>
        {
            if( err.code === 11000 ){ throw { code: 409, message: 'Couldn\'t create new entry of ' + this.constructor.name + ', ' + require('util').inspect( err.keyValue ) + ' already exists', args: { id, data, options }}}

            throw { code: 500, message: 'Couldn\'t create new entry of ' + this.constructor.name, args: { id, data, options }, mongoErr: err }
        });

        if( insert.insertedCount !== 1 ){ throw { code: 500, message: 'Couldn\'t create new entry of ' + this.constructor.name, args: { id, data, options }, mongoErr: err}}

        key && Object.defineProperty( entry, '__key', { writable: false, enumerable: false, value: key });
        delete entry.__model;

        this.#options.logger && this.#options.logger.log( 'create', this.#name, id, entry ).catch( NOOP );
    ...
    async get( id, properties, options = {})
    {
        ...
        {
            entry = await this.#get_aggregator.call( id, properties, options );
    ...
    async update( id, modification, options = {})
    {
        ...
        modification.$set['__model.updated'] = Date.now();
