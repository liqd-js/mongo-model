...
module.exports = class Model extends EventEmitter
{
    ...
    async update( id, modification, options = {})
    {
        ...
        do
        {
            ...
            entry = await this.#collection.findOneAndUpdate({ ...filter, $or: [{ '__model.lock': { $exists: false }}, { '__model.lock': { $lt: getLockTimeout()}}, { '__model.lock': options.key }]}, modification, options.upsert ? { upsert: true } : undefined ).then( r => r.value );
