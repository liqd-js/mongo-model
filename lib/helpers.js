...
module.exports.CompileStringRE = function CompileStringRE( obj )
{
    if( Array.isArray( obj ))
    {
        for( let i = 0; i < obj.length; ++i )
        {
            if( typeof obj[i] === 'string' && obj[i].startsWith('/'))// && filter[key].endsWith('/') )
            {
                obj[i] = eval( obj[i] ); // TODO lepsie
            }
            else if( typeof obj[i] === 'object' )
            {
                CompileStringRE( obj[i] );
            }
        }
    }
    else
    {
        for( let key in obj )
        {
            if( typeof obj[key] === 'string' && obj[key].startsWith('/'))// && filter[key].endsWith('/') )
            {
                obj[key] = eval( obj[key] ); // TODO lepsie
            }
            else if( typeof obj[key] === 'object' )
            {
                CompileStringRE( obj[key] );
            }
        }
    }
...
function getlatinAlternatives(c)
{
    let alternatives = c;

    if (LATIN[c])
    {
        alternatives += LATIN[c];

        for (let char in LATIN)
        {
            if (LATIN[char] === LATIN[c])
            {
                alternatives += char;
            }
        }
...
module.exports.EscapeREString = function EscapeREString( re )
{
    return ('' + re).split('').map(c => /^[A-Za-z0-9]+$/.test(c) ? c : (c === "\\000" ? "\\000" : "\\" + c)).join('');
}

module.exports.EscapeREStringLatin = function EscapeREStringLatin( re )
{
    return ('' + re).split('').map(c => /^[0-9]+$/.test(c) ? c : (LATIN[c] || /[a-zA-Z]/.test(c) ? '[' + getlatinAlternatives(c) + ']' : (c === "\\000" ? "\\000" : "\\" + c))).join('');
}
